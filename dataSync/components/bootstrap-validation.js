/* ========================================================= 
 * 
 * 修改bootstrap-validation.js，支持 bootstrap3
 *
 * check-type=
...
!function($) {
    ...
    $.fn.valid=function(options){
        ...
        //取出验证的
        $('input, textarea,select,div.multiChk', this).each(function () {
            var el = $(this), 
            ...
            if (!controlGroup.hasClass('success') && valid != null && valid.length > 0) {
   ...
   $.fn.validation.defaults = {
    ...
    var validateField = function(field, valid) { 
        ...
        for (i = 0; i < valid.length; i++) {
            ...
            for (j = 0; j < rules.length; j++) {
                ...
                if (flag == rule.name) {
                    var value;
                    if (el.attr('type') && el.attr('type')=='checkbox'){
                    ...
                    //针对复选框做处理
                    else if (el.is('div.multiChk')){
                    	value=el.find("input:checked").length>0 ? "1":"";
                    }
                    else{
                        value=el.val();
                    };
        ...
        controlGroup.addClass(error==false?'success':'error');
        var form = el.parents("form");
        if(form){
            var fstyle = isformstyle(form);
            if(fstyle == 0){
                controlGroup.find("#valierr").remove();
                if (el.parent().is(".input-append")){
    ...
    var validationForm = function(obj) {
        ...
        $(obj).find("select,input[type='file']").each(function(){
           var el = $(this);
      ...
      //针对复选框做处理
        $(obj).find("div.multiChk").each(function(){
        	var el=$(this);
        ...
        if (globalOptions.reqmark==true){
            if(fform_style==0){
                $(obj).find(".control-group>label").each(function(){
                    ...
                    el.append('<span class="asteriskField">*</span> ')
                });
            }
                ...
                $(obj).find('input, textarea,select,div.multiChk').each(function(){
                    var el = $(this);
                    var controlGroup = el.parents('.control-group');
